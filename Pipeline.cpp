...
Pipeline::Pipeline(Device* device, SwapChain* swapChain, RenderPass* renderPass) : device(device), swapChain(swapChain), renderPass(renderPass) {
    createGraphicsPipeline();
}

Pipeline::~Pipeline() {
    vkDestroyPipeline(device->getLogical(), graphicsPipeline, nullptr);
...
void Pipeline::createGraphicsPipeline() {
    ...
    std::array<VkVertexInputBindingDescription, 2> bindingDescriptions = {bindingDescription, instanceBindingDescription};
      ...
      rasterizer.cullMode = VK_CULL_MODE_NONE; // Disable face culling
        ...
        pipelineLayoutInfo.pSetLayouts = renderPass->getDescriptorSetLayout();
        ...
        if (vkCreateGraphicsPipelines(device->getLogical(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
            throw std::runtime_error("failed to create graphics pipeline!");
        }

        vkDestroyShaderModule(device->getLogical(), fragShaderModule, nullptr);
        vkDestroyShaderModule(device->getLogical(), vertShaderModule, nullptr);
    }
